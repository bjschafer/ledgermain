...
> = {
  ...
  "fighter:cad:craven-combatant:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; cad's other features union-cover every AT tier (Catch Off-Guard wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants flanking immunity (except from higher-level rogues/ninjas) while fighting defensively, no exploitable number",
  },
  ...
  "fighter:cad:treacherous-blow:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "replaces armor training 4, covered the same way — own ability grants an extra dirty-trick maneuver attempt on a confirmed critical hit, no independent number",
  },
  ...
  "fighter:cavern-sniper:quick-and-deadly:4": {
    ...
    level: 4,
    bucket: "subsystem",
    note: "wired: the 4th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability removes the Stealth-at-full-speed penalty and lets the cavern sniper apply poison as a swift action, no independent number",
  },
  ...
  "fighter:child-of-acavna-and-amaznen:eldritch-lore:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability alters skill points and adds class skills, no exploitable number",
  },
  ...
  "fighter:corsair:deck-fighting:2": {
    ...
    level: 2,
    bucket: "subsystem",
    note: "the 2nd-level bonus-feat instance it trades was vendor-paired to Bravery (the CSV level-matching quirk, now neutralized in MISPAIRED_TARGET_REMAP) and is wired via ARCHETYPE_TIER_REPLACEMENTS instead — own ability grants Cleave as a bonus feat, no independent number",
  },
  ...
  "fighter:corsair:improved-deck-fighting:6": {
    ...
    level: 6,
    bucket: "subsystem",
    note: "wired: the 6th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants Great Cleave as a bonus feat, no independent number",
  },
  ...
  "fighter:crossbowman:greater-deadshot:11": {
    ...
    level: 11,
    bucket: "situational",
    note: "replaces armor training 3; crossbowman's other features union-cover every AT tier (Deadshot wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability adds a real Dex-to-damage bonus, but scoped to a crossbow attack made as a readied action",
  },
  ...
  "fighter:crossbowman:pinpoint-targeting:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "replaces armor training 4, covered the same way — own ability grants Pinpoint Targeting as a bonus feat, no independent number",
  },
  ...
  "fighter:dragoon:skilled-rider:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants Mounted Combat and Skill Focus (Ride) as bonus feats, no independent number",
  },
  ...
  "fighter:dragoon:piercing-lance:11": {
    ...
    level: 11,
    bucket: "situational",
    note: "suppress-plus-backfill: dragoon's own Armor Training row above (flat, no further scaling) already carries only the kept tiers, so the tier this trades is simply absent from that schedule — own ability grants a bonus attack against a mounted rider's mount plus +4 to the hit's Ride-check DC, scoped to attacking a mounted opponent",
  },
  ...
  "fighter:dragoon:leaping-lance:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "suppress-plus-backfill, covered the same way as Piercing Lance — own ability doubles charge attack/AC modifiers when leaping 10 feet off the mount during a charge, a real but narrowly-scoped number",
  },
  ...
  "fighter:druman-blackjacket:blackjacket-tactics:4": {
    ...
    level: 4,
    bucket: "subsystem",
    note: "wired: the 4th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants an unmodeled teamwork-feat-sharing subsystem, no independent number",
  },
  ...
  "fighter:druman-blackjacket:amateurs:8": {
    ...
    level: 8,
    bucket: "situational",
    note: "wired: the 8th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — extends the unmodeled Blackjacket Tactics subsystem with a real +1 morale bonus on skill checks and saves, scoped to that subsystem's Blackjacket-only participation state",
  },
  "fighter:druman-blackjacket:superior-tactics:12": {
    ...
    level: 12,
    bucket: "subsystem",
    note: "wired: the 12th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a second shared teamwork feat through Blackjacket Tactics, no independent number",
  },
  "fighter:druman-blackjacket:esprit-de-corps:16": {
    ...
    level: 16,
    bucket: "situational",
    note: "wired: the 16th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a real +2 (conditionally +3) morale bonus on attacks, ability checks, skill checks, and saves, scoped to the unmodeled Blackjacket Tactics subsystem",
  },
  "fighter:eldritch-guardian:familiar:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a familiar, no independent number",
  },
  "fighter:eldritch-guardian:share-training:2": {
    ...
    level: 2,
    bucket: "subsystem",
    note: "the 2nd-level bonus-feat instance it trades was vendor-paired to Bravery (the CSV level-matching quirk, now neutralized in MISPAIRED_TARGET_REMAP) and is wired via ARCHETYPE_TIER_REPLACEMENTS instead — own ability lets the familiar use the guardian's combat feats, no independent number",
  },
  ...
  "fighter:foehammer:piledriver:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; foehammer's other features union-cover every AT tier (Sledgehammer wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants a single standard-action attack with a free bull rush/trip maneuver on a hit, no independent number",
  },
  ...
  "fighter:gloomblade:shadow-weapon:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "the wholesale 'replaces armor training' claim is now paired via a data-pipeline supplement (SUPPLEMENTAL_ARCHETYPE_FEATURE_PAIRING in supplements.ts), so the base Armor Training grant is correctly suppressed — grants a summoned/scaling weapon (no engine concept of a non-itemized weapon); its enhancement-bonus-on-the-weapon half stays unmodeled",
  },
  ...
  "fighter:lore-warden-pfs-field-guide:hair-s-breadth:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; lore warden's other features union-cover every AT tier (Maneuver Mastery wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants an Acrobatics check to negate a crit confirmation, no independent number",
  },
  "fighter:lore-warden-pfs-field-guide:swift-lore:14": {
    ...
    level: 14,
    bucket: "subsystem",
    note: "replaces armor training 4, covered the same way — own ability lets Know Thy Enemy be used as a swift action, no independent number",
  },
  ...
  "fighter:mobile-fighter:rapid-attack:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "suppress-plus-backfill: mobile fighter's own Armor Training row above (reduced 2-tier cadence) already carries only the kept tiers, so the tier this trades is simply absent from that schedule — own ability grants a full-attack-while-moving option, no independent number",
  },
  "fighter:mobile-fighter:fleet-footed:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "suppress-plus-backfill, covered the same way as Rapid Attack — own ability grants a real +10 speed increase plus Acrobatics take-10/take-20 riders, unwired numbers",
  },
  ...
  "fighter:opportunist:cunning-edge:4": {
    ...
    level: 4,
    bucket: "subsystem",
    note: "wired: five bonus-feat instances (4th, 8th, 12th, 16th, and 20th level) are subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a choose-from-list menu of edges (bombs, poison use, rogue talent, etc.), no single independent number",
  },
  ...
  "fighter:pack-mule:unobtrusive:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability only adds class skills, no number",
  },
  ...
  "fighter:phalanx-soldier:irresistible-advance:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "replaces armor training 4; phalanx soldier's other features union-cover every AT tier (Phalanx Fighting wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants a real shield-type-dependent bonus on bull rush/overrun CMB checks, conditional/narrowly-scoped per the honesty bar",
  },
  ...
  "fighter:rondelero-duelist:chopping-blow:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; rondelero duelist's whole-grant pairing already lives on Buckler Catch, and this file's own Armor Training row (L7, delayed onset) backfills the kept, delayed schedule, so the tier trade needs no separate handling — own ability grants a single standard-action falcata attack with a free sunder maneuver on a hit, no independent number",
  },
  ...
  "fighter:roughrider:ride-them-down:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "replaces armor training 4; roughrider's other features union-cover every AT tier (Armored Charger wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants a mounted full-attack-while-moving option, no independent number",
  },
  "fighter:roughrider:unavoidable-onslaught:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "replaces armor training 4, covered the same way — own ability removes the friendly-creature/difficult-terrain block on a mounted charge, no independent number",
  },
  ...
  "fighter:sensate:evasion:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; sensate's other features union-cover every AT tier (Uncanny Dodge wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants the rogue's evasion ability, no independent number",
  },
  "fighter:sensate:steady:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "replaces armor training 4, covered the same way — own ability lets the sensate center as a swift action, no independent number",
  },
  ...
  "fighter:siegebreaker:breaker-rush:1": {
    ...
    level: 1,
    bucket: "situational",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability is a real Str-bonus bludgeoning-damage number, scoped to bull rush/overrun maneuvers made without provoking",
  },
  ...
  "fighter:siegebreaker:breaker-momentum:2": {
    ...
    level: 2,
    bucket: "subsystem",
    note: "the 2nd-level bonus-feat instance it trades was vendor-paired to Bravery (the CSV level-matching quirk, now neutralized in MISPAIRED_TARGET_REMAP) and is wired via ARCHETYPE_TIER_REPLACEMENTS instead — own ability grants a free overrun attempt after a successful bull rush, no exploitable number",
  },
  ...
  "fighter:siegebreaker:disorienting-blow:8": {
    ...
    level: 8,
    bucket: "situational",
    note: "wired: the 8th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability is a real Fortitude-negated penalty on a foe's rolls after a hit, conditional/narrowly-scoped per the honesty bar",
  },
  ...
  "fighter:tactician:cooperative-combatant:11": {
    ...
    level: 11,
    bucket: "situational",
    note: "suppress-plus-backfill: tactician's own Armor Training row above (reduced 2-tier cadence) already carries only the kept tiers, so the tier this trades is simply absent from that schedule — own ability extends aid another to more allies with a real, aid-another-scoped +2 bonus choice",
  },
  "fighter:tactician:battle-insight:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "suppress-plus-backfill, covered the same way as Cooperative Combatant — own ability grants a real Intelligence-modifier insight bonus on an ally's attack rolls, a swift-action, resource-gated buff",
  },
  ...
  "fighter:thunderstriker:knockback-smash:11": {
    ...
    level: 11,
    bucket: "situational",
    note: "replaces armor training 3; thunderstriker's other features union-cover every AT tier (Strapped Shield wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability adds a real bonus equal to the buckler's enhancement bonus, scoped to buckler attacks",
  },
  ...
  "fighter:thunderstriker:buckler-defense:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "replaces armor training 4, covered the same way — own ability grants a real +1 shield bonus to AC, scoped to fighting with a weapon in both hands/each hand while retaining buckler use",
  },
  ...
  "fighter:titan-fighter:giant-weapon-wielder:1": {
    ...
    level: 1,
    bucket: "situational",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability is a real -2 attack penalty, scoped to wielding an oversized two-handed weapon",
  },
  ...
  "fighter:two-handed-fighter:piledriver:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "replaces armor training 3; two-handed fighter's other features union-cover every AT tier (Overhand Chop wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability grants a single standard-action attack with a free bull rush/trip maneuver on a hit, no independent number",
  },
  "fighter:two-handed-fighter:greater-power-attack:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "replaces armor training 4, covered the same way — own ability doubles the Power Attack damage bonus, scoped to Power Attack with a two-handed weapon",
  },
  ...
  "fighter:two-weapon-warrior:improved-balance:11": {
    ...
    level: 11,
    bucket: "situational",
    note: "replaces armor training 3; two-weapon warrior's other features union-cover every AT tier (Defensive Flurry wholesale-pairs the base grant), so the boolean pairing is exactly right — own ability is a real -1 reduction to two-weapon-fighting attack penalties",
  },
  ...
  "fighter:two-weapon-warrior:perfect-balance:15": {
    ...
    level: 15,
    bucket: "situational",
    note: "replaces armor training 4, covered the same way — own ability is a further real -1 reduction to two-weapon-fighting penalties, stacking with Improved Balance",
  },
  ...
  "fighter:unarmed-fighter:tough-guy:3": {
    ...
    level: 3,
    bucket: "situational",
    note: "wired: replaces armor training 1 only (RAW the fighter keeps tier 3 here; the vendored whole-grant pairing on this feature is neutralized in MISPAIRED_TARGET_REMAP, and ARCHETYPE_TIER_REPLACEMENTS substitutes the kept-tier count) — own ability grants a real DR/- (half fighter level), scoped to nonlethal damage or damage taken while grappled",
  },
  ...
  "fighter:unarmed-fighter:clever-wrestler:7": {
    ...
    level: 7,
    bucket: "subsystem",
    note: "wired: replaces armor training 2, substituted via ARCHETYPE_TIER_REPLACEMENTS (same mechanism as Tough Guy) — own ability removes grapple penalties the engine never modeled",
  },
  "fighter:unarmed-fighter:trick-throw:8": {
    ...
    level: 8,
    bucket: "subsystem",
    note: "wired: the 8th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a bonus dirty-trick maneuver attempt on a successful trip, no independent number",
  },
  "fighter:unarmed-fighter:takedown:12": {
    ...
    level: 12,
    bucket: "subsystem",
    note: "wired: the 12th-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a bonus trip maneuver attempt after a successful drag or grapple, no exploitable number",
  },
  ...
  "fighter:unarmed-fighter:sucker-punch:17": {
    ...
    level: 17,
    bucket: "subsystem",
    note: "wired: trades armor training 4 (the tier gained at 15th, though this feature is itself gained at 17th — the trade is fixed when chosen) via ARCHETYPE_TIER_REPLACEMENTS — own ability grants a bonus dirty-trick or trip maneuver attempt against a flat-footed or pinned foe, no independent number",
  },
  ...
  "fighter:unbreakable:tough-as-nails:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability grants Endurance and Die Hard as bonus feats, no independent number",
  },
  ...
  "fighter:unbreakable:quick-recovery:11": {
    ...
    level: 11,
    bucket: "subsystem",
    note: "wired: replaces armor training 3 via ARCHETYPE_TIER_REPLACEMENTS (unbreakable pairs nothing to the base grant, so RAW it keeps tiers 1-2 and loses only 3 and 4) — own ability shortens fatigue recovery to 15 minutes, no independent number",
  },
  ...
  "fighter:unbreakable:unlimited-endurance:15": {
    ...
    level: 15,
    bucket: "subsystem",
    note: "wired: replaces armor training 4, covered the same way as Quick Recovery — own ability softens exhaustion to fatigue (with a 1-hour reduction time), no independent number",
  },
  ...
  "fighter:ustalavic-duelist:duelist-stance:1": {
    ...
    level: 1,
    bucket: "situational",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability is a real scaling dodge bonus to AC, scoped to being unarmored/light-armored with a one-handed melee weapon and an empty off hand",
  },
  ...
  "fighter:varisian-free-style-fighter:martial-flexibility:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: four bonus-feat instances (1st, 6th, 10th, and 12th level) are subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS; this feature also trades Weapon Training wholesale (fighter:varisian-free-style-fighter is in WEAPON_TRAINING_REPLACEMENTS in archetypes.ts) — own ability grants the brawler's Martial Flexibility subsystem, no independent number",
  },
  ...
  "fighter:weapon-bearer-squire:weapon-rack:1": {
    ...
    level: 1,
    bucket: "subsystem",
    note: "wired: the 1st-level bonus-feat instance it trades is subtracted from the fighter's bonus-feat budget via ARCHETYPE_TIER_REPLACEMENTS — own ability lets the knight/squire pair retrieve or pick up items without provoking, no exploitable number",
  },
  "fighter:weapon-bearer-squire:swift-sharpening:2": {
    ...
    level: 2,
    bucket: "situational",
    note: "the 2nd-level bonus-feat instance it trades was vendor-paired to Bravery (the CSV level-matching quirk, now neutralized in MISPAIRED_TARGET_REMAP) and is wired via ARCHETYPE_TIER_REPLACEMENTS instead — own ability is a real +1 circumstance bonus on the next attack roll with a sharpened weapon, scoped to a full-round sharpening action",
  },

...
export interface SituationalFeatEffect {
  ...
  damage?: number;
  /**
   * True when only HALF this `damage` bonus applies to an off-hand attack —
   * Power Attack and its light-weapon analog Piranha Strike say so explicitly
   * (PF1 CRB p. 131). Only consulted for the off-hand line of a two-weapon
   * roll; the primary hand always gets the full `damage`.
   */
  damageHalvedOffHand?: boolean;
...
export const SITUATIONAL_FEAT_EFFECTS: Readonly<Record<string, SituationalFeatEntry>> = {
  ...
  "power-attack": {
    ...
    effect: (ctx, option) => {
      ...
      return { attack: -p, damage, damageHalvedOffHand: true };
    },
  },
  ...
  "piranha-strike": {
    ...
    effect: (ctx) => {
      const p = 1 + Math.floor(ctx.bab / 4);
      return { attack: -p, damage: 2 * p, damageHalvedOffHand: true, note: "light weapons only" };
    },
  },

  // The two-weapon fighting chain is deliberately absent here: fighting with
  // two weapons is a MODE (available to anyone, feats or not), not a feat
  // attachment, so its penalty table and off-hand sequence live in
  // `two-weapon-fighting.ts` and are applied by a saved roll's two-weapon
  // toggle, which picks up every owned chain feat automatically.

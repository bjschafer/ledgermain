...
const MONK_KI_POWER_NAME_ALIASES: Record<string, string> = {};

function normalizeKiPowerName(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
...
export interface MergedMonkKiPowerEntry extends MonkKiPowerDef {
  /** Ability-type suffix as published, e.g. "(Su)", "(Ex)" — absent for the (currently none) hand-authored-only case. */
  nameSuffix?: string;
...
function vendoredToDef(entry: MonkKiPower): MergedMonkKiPowerEntry {
  return {
    id: entry.id,
    name: entry.name,
    nameSuffix: entry.nameSuffix,
    // NOT `entry.level` — not a level-gate (see `MonkKiPower.level`'s doc
    // comment). A vendored-only entry gets the same 4th-level floor every
    // ki power shares, rather than a fabricated per-entry minimum.
    minLevel: 4,
...
/**
 * Resolve a picked ki-power id (`doc.build.monkKiPowers` entries) to its
 ...
 * `collect.ts`/`archetypes.ts` instead of indexing `MONK_KI_POWERS` directly,
 * so a vendored-only pick resolves to a real (display-only) definition
 * rather than being silently dropped.
 */
export function resolveMonkKiPower(id: string, refData: RefData): MonkKiPowerDef | undefined {
  const hand = MONK_KI_POWERS[id];
  if (hand) return hand;
  const vendored = refData.monkKiPowers?.[id];
 ...
 * picker's "M" badge.
 */
export function mergedMonkKiPowerCatalog(refData: RefData): MergedMonkKiPowerEntry[] {
  const handByNormName = new Map<string, MonkKiPowerDef>();
  for (const p of KI_POWER_LIST) {
    handByNormName.set(normalizeKiPowerName(MONK_KI_POWER_NAME_ALIASES[p.id] ?? p.name), p);
  }

  const vendored = Object.values(refData.monkKiPowers ?? {});
  const merged: MergedMonkKiPowerEntry[] = [];
  for (const v of vendored) {
    const handMatch = handByNormName.get(normalizeKiPowerName(v.name));
    merged.push(
      handMatch
        ? { ...handMatch, nameSuffix: v.nameSuffix, description: v.description, sources: v.sources }
        : vendoredToDef(v),
    );
  }
  return merged;

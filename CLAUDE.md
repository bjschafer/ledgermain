...
- **Content gaps are tracked in two places that drift silently — update both.** Issue #74 is the canonical inventory of content Ledgermain doesn't cover; `apps/web/src/model/coverageNotes.ts` is its player-facing mirror, rendered as the "What's Not Covered" panel in Settings. Filling a gap means deleting it from both, and finding one means adding it to both. Nothing enforces the pairing, and a stale mirror actively lies to players about what the app supports. Keep #74 a plain inventory of what's _missing_ — no phase/status narrative, and cite it as a bare `#74` (never "#74 Phase 3b"; the phases are gone).

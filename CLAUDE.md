...
- **Content gaps have one source of truth: `apps/web/src/model/coverageNotes.ts`** (rendered as the "What's Not Covered" panel in Settings; purely internal gaps go in its `INTERNAL_GAPS` array, which the panel never shows). Issue #74 is a generated mirror: run `bun run coverage:issue` and sync its output to the issue body — never hand-edit either side independently, and never edit the issue without user confirmation. Filling a gap means deleting its entry; finding one means adding it. Keep #74 a plain inventory of what's _missing_ — no phase/status narrative, and cite it as a bare `#74`.

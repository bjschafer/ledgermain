...
**Precision measurement (the point of the pilot):** re-deriving Weapon Master (Armor Training reflavor), Archer (Hawkeye's Perception half), and Two-Handed Fighter (Overhand Chop, notes-only) from prose alone reproduced the hand table's entries exactly — same targets, same formulas, same situational calls on every other feature of those 3 archetypes. Zero wrong-number or wrong-target misses; zero cases where hand-authoring caught something extraction missed, or vice versa, within the ground-truth set. (Caveat: see finding 4 above — this isn't a fully blind result.) Across the other 64 fighter archetypes, 17 new `numeric` entries were extracted (7 `high` confidence, 9 `medium`, one entry — Sun-Bronzed Skin — extracted and then correctly self-rejected per finding 2), 5 of which were independently spot-verified against aonprd.com and matched the vendored prose and this agent's formulas exactly (noted in `archetype-classification.ts` as "spot-verified against aonprd.com," still machine-tier in code — promotion mechanics are a future decision, per the issue). 39 features were classified `blocked` (a real, if narrow, composition-safety bucket this pilot needed to invent, not previously documented outside the Ironskin Monk case). The remaining 325 features split `situational`/`subsystem` via a disclosed, heuristic-assisted pass (documented in `archetype-classification.ts`'s own header) rather than individual hand-verification — a deliberate scoping choice, since that boundary doesn't affect engine correctness (neither bucket emits a `Change`).

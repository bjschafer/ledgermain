...
Four real gaps/quirks were found during this audit:
...
- **AC Bonus (Wis-to-AC) only correctly gates on armor, and double-counts in CMD when unarmored.** Its vendored formula checks three conditions (`lt(@shield.type, 1)`, `lt(@armor.type, 1)`, `lt(@attributes.encumbrance.level, 1)`) before applying `wisMod + floor(unlevel/4)` to both `ac` and `cmd`. `@armor.type` is correctly populated by `rolldata.ts` from equipped gear, so the "not wearing armor" case works. `@shield.type` is never populated anywhere in `rolldata.ts` (missing path → `0`, i.e. always reads as "no shield"), and `@attributes.encumbrance.level` is hardcoded to `0` (no encumbrance model yet — issue #16's territory). So a monk wielding a shield or carrying a heavy load incorrectly still receives the bonus. Building shield-slot tracking or encumbrance is out of scope here. Additionally, when the bonus DOES apply (unarmored), it is double-counted in CMD specifically: the same untyped "generic"-category change already feeds `cmd` once via `computeAc`'s `cmdAcBonus` (derived from any `dodge`/`deflection`/`generic` `ac.components`), and AC Bonus also carries a second, separate `cmd`-targeted change with the identical formula, adding it again. AC itself (`ac.normal`/`ac.touch`) is unaffected — only CMD is doubled. **This double-count is NOT monk-specific** — the same `cmdAcBonus`-vs-explicit-`cmd`-change collision already exists for at least two other vendored items (Iron Mask, both masterwork and common) and one buff (Deflection Aura); monk's AC Bonus is just the first *class feature* example. Pre-existing, cross-cutting bug in `computeAc`'s CMD derivation — deliberately not touched here pending owner input on the right fix (e.g. dedup by source, or drop `cmdAcBonus` auto-derivation and require every vendored bonus to carry its own explicit `cmd` change). See the "armored vs. unarmored" fixture test block in `compute.test.ts`.

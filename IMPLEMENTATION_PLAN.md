...
Three real gaps/quirks were found during this audit — none fixed here, all out of scope for a vendoring step:

- **Maneuver Training's CMB correction is inflated.** Its vendored formula (`@class.unlevel - @attributes.bab.total`, untyped, target `cmb`) is meant to swap medium-BAB CMB for full-monk-level CMB. But `packages/engine/src/rolldata.ts`'s `buildRollData` never populates an `attributes.bab` path at all, and `collectModifiers()` runs *before* `compute()` derives `bab` as a local variable — so `@attributes.bab.total` always resolves to the formula DSL's missing-path default of `0`, and the change nets to `unlevel - 0` instead of `unlevel - actualBab`. A monk's CMB ends up higher than the SRD value (correct in direction, wrong in magnitude). Fixing this would mean either reordering `compute()` (deriving BAB before the first `collectModifiers()` pass) or adding a second collect pass — a real engine restructuring, not appropriate for this step. See the fixture test in `packages/engine/test/compute.test.ts` ("Maneuver Training's CMB correction...") for the worked numeric example.
- **AC Bonus (Wis-to-AC) only correctly gates on armor.** Its vendored formula checks three conditions (`lt(@shield.type, 1)`, `lt(@armor.type, 1)`, `lt(@attributes.encumbrance.level, 1)`) before applying `wisMod + floor(unlevel/4)` to both `ac` and `cmd`. `@armor.type` is correctly populated by `rolldata.ts` from equipped gear, so the "not wearing armor" case works. `@shield.type` is never populated anywhere in `rolldata.ts` (missing path → `0`, i.e. always reads as "no shield"), and `@attributes.encumbrance.level` is hardcoded to `0` (no encumbrance model yet — issue #16's territory). So a monk wielding a shield or carrying a heavy load incorrectly still receives the bonus. Building shield-slot tracking or encumbrance is out of scope here. Additionally, when the bonus DOES apply (unarmored), it is double-counted in CMD specifically: the same untyped "generic"-category change already feeds `cmd` once via `computeAc`'s `cmdAcBonus` (derived from any `dodge`/`deflection`/`generic` `ac.components`), and AC Bonus also carries a second, separate `cmd`-targeted change with the identical formula, adding it again. AC itself (`ac.normal`/`ac.touch`) is unaffected — only CMD is doubled. See the "armored vs. unarmored" fixture test block in `compute.test.ts`.
...
- **Unarmed Strike's vendored `bonusFeats` change is mis-attributed.** Its `changes[]` entry (flat `"1"`, target `bonusFeats`) appears to represent the automatic Improved Unarmed Strike grant every monk gets at L1, but `classBonusFeats()`'s fixed-grant filter only skips a class feature when its *name* matches a real feat's name exactly (`"Unarmed Strike"` != `"Improved Unarmed Strike"`), so it isn't recognized as an auto-granted feat and instead inflates `expectedFeatCount`'s free bonus-feat-slot budget by +1 at every monk level. Closely tied to Unarmed Strike, which is explicitly step 2 scope (hand-authoring unarmed damage die + Flurry of Blows) — left unfixed and documented here instead. See `apps/web/test/feats.test.ts`'s "Monk Bonus Feat + a vendored-data quirk" block for the worked numeric example.

...
It does not choose or edit the pin — that's the deliberate, judgment-call part of the procedure (step 1 above) and stays a manual edit to `config.ts` (or `supplements.ts`). It only automates the mechanical fetch → build → fmt sequence that follows.

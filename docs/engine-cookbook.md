...
1. **BAB and base size first** (`computeBab()`, called at the top of the function). Base attack bonus (a class's per-level combat-effectiveness score) only depends on class levels, so it's computed before anything else needs it as an input. A monk's Flurry of Blows (`flurry.ts`) is resolved in the same place, off a second set of per-class BAB tiers: the chained monk flurries at a base attack bonus equal to her monk level, so the substitution has to happen where the tiers are still in hand. Base size comes from the race.
2. **Boot pass** -- `buildRollData()` (`rolldata.ts`) assembles the `@data.path` context formulas evaluate against, using base ability scores; `collectModifiers()` (`collect.ts`) runs every subsystem collector in `collect/` (race, gear, class features, archetypes, active buffs, conditions, traits, and ~30 more build-choice collectors) and evaluates each `Change`'s formula into a flat list of `CollectedModifier`s; `computeAbilities()` folds those through the stacking resolver to get real ability totals. This is a bootstrap because some formulas (e.g. carrying capacity, encumbrance-driven AC caps) need final Strength before they can run, but final Strength needs the same collect pass encumbrance depends on -- so the boot pass breaks the circularity by collecting once against base scores.
...
6. **Attack, AC, CMB/CMD** -- `computeAttackLines()` (base melee/ranged attack bonus, plus the flat armor/shield penalty the per-weapon lines reuse), `computeAc()` (armor class -- how hard the character is to hit), and `computeManeuvers()` (CMB/CMD -- the grapple-and-trip stat) are all built from `collected` plus the ability/size values above.
7. **Initiative, HP, speeds** -- straightforward modifier sums via `resolveStack`, and `computeSpeeds()` for the per-mode `applySpeedTarget` fan-out plus the armor/encumbrance land-speed reduction.
8. **Arcane spell failure, skills, per-weapon attacks, kinetic blasts** -- `computeSkills()` handles the parameterized-skill fan-out (Craft/Profession/Perform subtypes); `computeWeaponAttacks()` builds one resolved attack line per `build.weapons` entry, plus a flurry sequence for each unarmed strike or monk weapon when the character has a flurry; `computeKineticBlasts()` (`kinetic-blast.ts`) does the same for a kineticist's blasts, which have no gear entry to hang off and scale with the burn currently held, plus the per-activation loadout (`live.kineticistBlastLoadout`) resolved through `kineticist-infusions.ts`.
9. **Defenses, senses, active polymorph form** -- `computeDefenses()` (DR/energy resistance/spell resistance), `computeSenses()` (darkvision etc.), and `computeActiveForm()`, which resolves the natural-attack lines for a Wild Shape/Beast Shape transformation when `live.activeForm` is set.
...
Two collect passes, one stacking resolver, one formula evaluator -- that combination is the entire engine. Everything else in the package is _data_ (tables of `Change`s) that flows through one of `collectModifiers()`'s subsystem collectors.
...
- `collect/shared.ts`'s `evalChange` helper wraps every single `Change` evaluation in a try/catch and silently drops a modifier on `null`/`NaN`/thrown syntax error. A malformed formula anywhere in the vendored data degrades that one bonus to "not applied," never a crashed sheet.
...
3. **Collection.** One function per build-choice field, each taking the shared `CollectContext` and pushing into its `out`; `collect.ts` holds nothing but the ordered `SUBSYSTEMS` list that calls them. `collectOracleCurse` (`collect/class-powers.ts`) is representative of the whole pattern:
   ```ts
   ...
   ```
   Note the gating: a non-oracle with a stale `oracleCurse` field (e.g. after switching classes) contributes nothing, because the level check runs first. This "gate on the granting class's level, never crash on an unknown/stale id" shape is universal across every collector in `collect/` -- copy it. A new subsystem is a new function in whichever `collect/*.ts` file it belongs to (a new file if none fits) plus one entry in `collect.ts`'s `SUBSYSTEMS`; forgetting the second half is the one way to write a collector that silently never runs.
...
Most of the 40-odd per-class modules (`arcanist-exploits.ts`, `witch-hexes.ts`, `rage-powers.ts`, `magus-arcana.ts`, `alchemist-discoveries.ts`, ...) follow this exact five-step shape: schema field → definition table with `changes: Change[]` → a gated collector in `collect/` registered in `SUBSYSTEMS` → an existing consumer in `compute.ts` (or, if none exists yet, a new one plus a `targets.ts` entry) → a fixture test. Many entries in these tables are `changes: []` with only `contextNotes` (a prose reminder shown in the UI, never fed through the collectors) -- that's the honest choice for an ability that's genuinely conditional/situational/triggered rather than a flat always-on number (see `judgments.ts`'s Resistance/Smiting entries, or almost any `displayOnly` cavalier order ability in `cavalier-orders.ts`). Don't invent a `Change` for something that only sometimes applies; a note is more honest than a wrong number.
...
Buffs (temporary magical/mundane effects) and conditions (afflictions like shaken or prone) are largely **already vendored** -- `RefData.buffs`/the `CONDITIONS` table in `conditions.ts` carry their own `changes[]` in the same `{ formula, target, type }` shape as everything else, because Foundry's own data model for these already matches this engine's typed-modifier model (see `docs/design.md` §4: "Buffs already ARE the typed-modifier model"). `collect/live-state.ts`'s `collectActiveBuffs` applies `doc.live.activeBuffs[*].changes` directly; `collectConditions` applies `CONDITIONS[condId].changes` directly. Most of the time, adding a new buff means nothing in the engine -- it's a data problem (is the buff correctly vendored?), not a code problem.
...
3. `collectActiveBuffs` applies `BUFF_CHANGE_PATCHES[buff.name] ?? []` alongside the buff's own vendored `changes[]` automatically -- no new wiring needed once the entry exists.
...
Buff-gated build choices are the other buff-adjacent pattern worth knowing: a hand-authored `Change` can carry `activeWhenBuff: { buffIds?, effectTags?, requiredEffectTags? }`, consulted by `collect/shared.ts`'s `buffGateSatisfied()` through the context's `gateOpen`. Use this when a permanent build choice's bonus should only apply while a _separate_ buff is active -- e.g. a barbarian rage power that only works "while raging" (`rage-powers.ts`'s Raging Climber). `buffIds`/`effectTags` are one OR group; `requiredEffectTags` is a separate AND group whose every entry must also be active, which is how a conjunction like "Crane Style is on AND you are fighting defensively or using total defense" is expressed without inventing a combined toggle (`feat-effects.ts`'s `crane-style`). A gated-but-inactive change is simply omitted from the collected list, not pushed through with a forced `applied: false` -- see `buffGateSatisfied`'s doc comment for why.
...
3. **`ARCHETYPE_TIER_REPLACEMENTS`** (`archetype-tier-replacements.ts`) -- for the residue where the base grant must stay live with individual tiers removed (Unbreakable, unarmed fighter): `collect/class-features.ts` substitutes the kept-tier count for the vendored formula. The same table's `"bonus feat"` kind subtracts single traded-away bonus-feat instances from the class's slot budget (`apps/web`'s `model/feats.ts`), unioned with vendored leveled `replacesSlot: { kind: "bonus feat" }` entries (monk/warpriest/swashbuckler archetypes carry those upstream). Entries key off the feature's own id, gate on the replaced tier's _gain_ level (not the feature's grant level), and claim conflict slot keys so two archetypes trading the same tier hard-block in the picker.

### 3.5 Add a granted-power patch (domain/school/inquisition)

A cleric/inquisitor domain, wizard arcane school (or focused school), and inquisitor inquisition all resolve their granted powers through one shared function, `collectGrantedFeatures()` (`archetypes.ts`) -- used for the class-feature display list and uses/day resource pools. It has the same shape `collectModifiers` does: one function per source under `granted-features/`, run from the ordered `GRANT_SOURCES` list, so adding a new source of grants means a function there plus one entry in that list. Unlike a base class feature's own `changes[]` (§3.1, walked directly by `collectGrantedClassFeatures`), nothing walked a _granted_ power's `changes[]` before `granted-power-effects/`: a domain power whose text plainly promises a bonus could sit inert forever.
...
`refdata-index.ts` holds the tag- and id-keyed maps instead: `classByTag`, `classFeatureByTag`, `domainByTag`, `subdomainByTag`, and `archetypeFeaturesOf` (that archetype's features, in collection order -- several callers apply swaps last-wins over it, so the order matters). The index is memoized per `RefData` object in a `WeakMap` rather than built by a loader, because RefData reaches the engine three ways and only one of them goes through a loader this repo owns. Every collection read tolerates a missing one, since tests and callers both hand in partial `RefData` stubs.

Reach for a helper rather than writing a fresh scan; if you need a lookup shape that isn't there, add it to the index instead of scanning at the call site. `packages/engine/test/computePerfCanary.test.ts` is the canary that notices when this stops being true -- a maximal character (twenty classes, eight archetypes, a pick from every catalog) against a deliberately loose wall-clock budget. It exists to catch an order-of-magnitude drift, not to benchmark; widen its threshold only for a measured, understood change.
...
Alongside the fixtures, a handful of tests check conventions rather than numbers. They cost nothing to run and fail loudly when a recipe above is followed almost-correctly:

- **`targetRegistration.test.ts`** parses every `src/` file and fails on a hand-authored `Change` whose `target` is not in `targets.ts`. An unregistered target is collected and dropped: the number never reaches the sheet and nothing complains. If the effect genuinely has no sheet line to land on (`reach`, `concentration`, `critConfirm`), it belongs in `contextNotes`, where the player at least reads the reminder.
- **`effectTableCollisions.test.ts`** asserts the hand-authored and machine-extracted tables for feats and archetype features share no key, and the same for the three by-origin granted-power tables. A shared key means one entry is unreachable, and a fixture written against it passes for the wrong reason.
- **`formulaFuzz.test.ts`** throws generated and corrupted formulas at the DSL. `parseFormula` and `tryEvaluateFormula` may only ever raise `FormulaSyntaxError` -- that is the exception `collect.ts` and the homebrew change editor catch, and anything else is a crash on data a bump could introduce.
- **`bun run knip`** reports unused exports and dependencies across the workspace. Non-blocking in CI: the baseline is large and mostly harmless, so treat it as a reading, not a gate.

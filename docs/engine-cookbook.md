...
8. **Arcane spell failure, skills, per-weapon attacks** — `computeSkills()` handles the parameterized-skill fan-out (Craft/Profession/Perform subtypes); `computeWeaponAttacks()` builds one resolved attack line per `build.weapons` entry.

...
| `GET /api/me/shares`                            | bearer | `{ shares: [{ token, characterId, kind, createdAt }] }`, newest first. The only route that lists share tokens.                                                                                  |
| `POST /api/me/shares`                           | bearer | Body `{ characterId, kind: "snapshot" \| "live" }`. 201 with the new share; 404 if the character isn't synced, 403 at 50 shares, 429 over 60 an hour.                                           |
| `DELETE /api/me/shares/:token`                  | bearer | Revoke: deletes the share's keys. 204, idempotent, and a no-op on another owner's token.                                                                                                        |
| `GET /api/shared/:token[?since=<version>]`      | none   | The share link's read. `{ kind, createdAt, doc }` with `ownerId` stripped, `cache-control: no-store`. A live share answers 204 when `since` is still current. Every miss is the same 404.       |

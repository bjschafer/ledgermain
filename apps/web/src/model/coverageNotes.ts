...
export const COVERAGE_NOTES: readonly CoverageNote[] = [
  ...
  },
  {
    ...
    note: "Every race's published alternates are browsable, and picking one applies whatever bonuses it spells out as numbers. What differs is the trade: for twenty-five races — the seven core races plus sylph, the featured aasimar, tiefling, dhampir, kitsune, ratfolk, and tengu, and now ifrits, oreads, undines, drow, kobolds, duergar, hobgoblins, goblins, fetchlings, catfolk, and vine leshies — taking an alternate also retires the standard trait it replaces (a drow trading away its sleep immunity really loses it; a kobold swapping its scales loses the natural armor). One honest exception: a heritage's different ability-score spread is still yours to apply, since the entry only describes it as text. For the remaining races the swap is yours to make: the entry names what it replaces, but nothing takes that standard trait away for you.",
    issueDetail:
      "860 vendored across all 80 races, 252 of them carrying structured `changes` that `collect.ts` applies live (plus `openChanges` once a target is chosen). Replacement enforcement: the hand-authored 8-race table via `suppressTargets`, plus seventeen races via `VENDORED_STANDARD_TRAIT_TARGETS` (a verified standard-trait-name → `Race.changes`-target map consumed by the vendored loop; heritage \"Base Statistics\" swaps deliberately unmapped — no structured replacement stats to swap in, with Vine Leshy's Agile the one audited exception since it ships a genuine full replacement spread). Deliberate non-mappings (speed traits living in `race.speeds`, contextNote-only traits, Drow SR with no structured field, Goblin's internally inconsistent Oversized Goblins data) are documented at the map. All other races' vendored entries name replaced traits with no verified target mapping, so they still apply on top.",
  },
  {

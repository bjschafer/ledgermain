...
export const COVERAGE_NOTES: readonly CoverageNote[] = [
  ...
  },
  {
    ...
    note: 'Every published entry in every one of these lists is browsable, searchable, and fully written up with level requirements flagged — rage powers, witch and shaman hexes, magus arcana, arcanist exploits, rogue, slayer, investigator, and vigilante talents, ninja tricks, mesmerist tricks and bold stares, alchemist discoveries, monk ki powers and style strikes, and phrenic amplifications. Entries that move numbers on your sheet automatically are marked with an "M" — rogue talents that grant a specific feat apply it for you, rage powers that grant a while-raging number apply it whenever you rage, and rage powers that make you choose an energy type when you take them (Energy Resistance, Draconic Blood, the Elemental Blood chain) now offer that choice in the picker and apply it for real. Everything else shows its rules text for you to apply at the table.',
    issueDetail:
      "Full hand-table parity across every pick list (issue #74): rage powers 243 (244 vendored rows less one literal Guarded Stance duplicate), rogue talents 234, alchemist discoveries 168, witch hexes 104, vigilante talents 81 + 46 social, arcanist exploits 73, investigator talents 67, ninja tricks 65, magus arcana 64, monk ki powers 44, mesmerist tricks 44, slayer talents 43, phrenic amplifications 31, mesmerist bold stares 24, shaman hexes 16, monk style strikes 15. Rage powers carry 31 promoted entries: 28 via the #75 while-raging gate (totem resistances and DR, Beast Totem's scaling natural armor, darkvision/low-light/scent grants incl. Lesser Moon Totem's additive `operator: \"add\"` darkvision, the Linnorm Death Curses' ungated +1 melee damage, Unrestrained Rage's paralysis immunity), plus 3 unlocked by the choose-one mechanism (`RagePowerDef.choice`/`choiceChanges` + `build.pickChoices`): Energy Resistance, Elemental Blood, Greater Elemental Blood (Draconic Blood's chosen resistance also rides it). The mechanism is rage-power-only so far — the oracle revelations' chosen-energy entries (defyElements, elementalAegis, draconicResistance) and other tables' nested picks remain display-only pending the same wiring. Other candidates blocked on missing mechanisms stay recorded inline in their table files (e.g. Improvised Weapon Proficiency's improvised-penalty model).",
  },
  {

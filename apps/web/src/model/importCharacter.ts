 ...
 * Kept separate from the migration chain since this is the one place untrusted
 * JSON enters the system — a malformed or foreign blob must be rejected before
 * it ever reaches Dexie or the engine.
 */
import type { CharacterDoc } from "@pf1/schema";

import { migrateImportedDoc } from "./migrations.js";

/**
 * Parse and validate an imported character export, then run every migration
 * step over it so older exports still load. Unlike the Dexie read path this
 * ignores the file's own `schemaVersion` — see `migrateImportedDoc`. Throws a
 * descriptive Error when the shape doesn't look like a character document, so
 * the import UI can explain the rejection.
 */
export function parseImportedDoc(value: unknown): CharacterDoc {
  ...
  return migrateImportedDoc(value);
}

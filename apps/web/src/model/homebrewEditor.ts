/**
 * Form-state <-> `Race`/`Feat` mapping for the homebrew authoring UI (phase 2
 * of homebrew content support — see `model/homebrew.ts` for the phase-1
 * doc-embedded storage + `RefData` overlay this builds on top of).
 *
 ...
 * `HomebrewFeatEditor.tsx`) stay thin wrappers around this module.
 */
import type { AbilityId, Feat, Race, SizeId, SkillId } from "@pf1/schema";

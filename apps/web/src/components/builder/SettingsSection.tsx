    ...
    <>
      ...
      <Panel title="HP Mode" step="⚙">
        ...
        <p className="hint" style={{ marginTop: 10, fontSize: 12 }}>
          ...
          {hpMode === "rolled" && "L1 = max HD; enter your rolls per level in the Hit Points panel."}
        </p>
      </Panel>
      ...
      <Panel title="Hero Point Cap" step="⚙">
        <p className="hint" style={{ marginBottom: 12 }}>
          Override the maximum number of hero points this character may hold at once. Default:{" "}
          {HERO_POINT_CAP}.
        </p>
      ...
      <Panel title="Manual Stat Overrides" step="⚙">
        <p className="hint" style={{ marginBottom: 12 }}>
          Force a derived stat to a specific value. Leave blank to use the computed value.
          The breakdown shows the override as a separate component.
        </p>
          ...
          {STAT_OVERRIDE_KEYS.map((key) => {
            ...
            return (
              <div key={key} className="stat-override-row">
                ...
                <NumberField
                  className="num stat-override-input"
                  value={override ?? computed ?? 0}
                  min={-999}
                  max={99999}
                  onCommit={(n) =>
                    update((d) => setStatOverride(d, key, Number.isNaN(n) ? null : n))
                  }
...
function resolveComputed(key: StatOverrideKey, sheet: BuilderProps["sheet"]): number | null {
  switch (key) {
    case "hp.max": return sheet.hp.max;
    case "ac.normal": return sheet.ac.normal;
    case "speeds.land": return sheet.speeds.land ?? null;
    case "initiative.total": return sheet.initiative.total;
    case "bab": return sheet.bab;
    case "cmd": return sheet.cmd;
    case "cmb": return sheet.cmb;
    case "saves.fort.total": return sheet.saves.fort.total;
    case "saves.ref.total": return sheet.saves.ref.total;
    case "saves.will.total": return sheet.saves.will.total;
    default: return null;
  }
}

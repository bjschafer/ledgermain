		...
		<Panel
			...
			right={
				<span className={`budge${over ? " over" : ""}`}>
					ranks <b>{budget.spent}</b> / {budget.total} ·{" "}
					<b>{budget.remaining}</b> left
				</span>
			}
		>
			{maxRank === 0 ? (
				<p className="empty">
					Add a class first — skill ranks come from class levels.
				</p>
			) : (
				<>
					<input
						...
						{skills.map((s) => (
							<div className="skill-row" key={s.id}>
								...
								<span
									...
									title={
										s.usable ? undefined : "trained only — no ranks invested"
									}
								>
									{signed(s.total)}
								</span>

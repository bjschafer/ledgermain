...
export function ResourcesPanel({ doc, sheet, refData, update }: BuilderProps) {
  ...
  return (
    <Panel
      ...
      step="rs"
      right={
        <button type="button" className="btn-ghost rest" onClick={() => update((d) => restAllResources(d))}>
          Rest (full)
        </button>
      }
    >
      {!hasAny ? (
        <div className="empty">No pools. Add spell slots or item charges below.</div>
      ) : (
      ...
      <div className="res-add">
        ...
        <input
          ...
          aria-label="Max"
          value={poolMax}
          onChange={(e) => setPoolMax(e.target.valueAsNumber)}
        />
        <button
          type="button"
  ...
  return (
    <div className="res-row">
      ...
      <div className="res-btns">
        <button type="button" className="btn-ghost" onClick={onDrain} disabled={left <= 0} aria-label={`spend ${name}`}>
          −
        </button>
        <button type="button" className="btn-ghost" onClick={onRestore} disabled={left >= max} aria-label={`restore ${name}`}>
          +
        </button>
        {onRemove ? (
          <button type="button" className="btn-ghost" onClick={onRemove} aria-label={`remove ${name}`}>
            ✕
          </button>
        ) : null}
      </div>

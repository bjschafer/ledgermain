...
const TARGETS = [
  "attack", "mattack", "rattack", "ac", "allSavingThrows", "fort", "ref", "will",
  "str", "dex", "con", "int", "wis", "cha", "init", "cmb", "cmd", "skills",
];
const TYPES = ["untyped", "enh", "morale", "luck", "sacred", "competence", "dodge", "deflection", "resistance", "circumstance"];
      ...
      right={
        <div className="round-ctl">
          ...
          <div className="lvl-stepper">
            <button type="button" onClick={() => setStep((s) => Math.max(1, s - 1))} aria-label="fewer">−</button>
            <span className="lvl num">{step}</span>
            <button type="button" onClick={() => setStep((s) => s + 1)} aria-label="more">+</button>
          </div>
        </div>
      }
    >
                ...
                <div className="buff-changes num">
                  {b.changes.map((c, i) => (
                    <span key={i} className="buff-change">
                      {c.target} {c.type ? <em>[{c.type}]</em> : null} {formulaHint(c.formula)}
                    </span>
                  ))}
                </div>
              </div>
...
function CustomBuffForm({
  ...
  onAdd: (name: string, target: string, type: string, value: number, rounds: number | undefined) => void;
}) {

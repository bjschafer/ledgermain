...
export function AfflictionsPanel({ doc, sheet, update }: BuilderProps) {
	...
	return (
		<Panel
			...
			right={anyActive ? <span className="hint">{active.length + (totalNeg > 0 ? 1 : 0)} active</span> : undefined}
		>
			{!anyActive ? (
				<div className="empty">
				...
				<ul className="cond-notes affliction-warnings">
					...
					{dying ? (
						<li className="affliction-warn">
							<b>Negative levels</b> ({totalNeg}) have reached the character's Hit Dice —
							the character dies (PF1 RAW).
						</li>
					) : null}
				</ul>
			)}
			...
			<div className="hint">
				Temporary negative levels allow a Fortitude save to remove each one 24h after
				being gained (display only — no timer is tracked here).
			</div>
...
function AfflictionRow({
	...
	const sub =
		kind === "drain"
			? `drain · score ${signed(-points)}`
			: `${kind} · ${signed(-modDelta)} mod`;

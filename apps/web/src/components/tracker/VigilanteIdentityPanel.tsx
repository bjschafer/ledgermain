...
/**
 * Vigilante dual-identity chip — display-forward only, per
 * `live.vigilanteIdentity`'s doc comment: no numeric consequences are wired to
 * this toggle. Several vigilante talents (Renown, Social Grace, Loyal Aid,...)
 * carry identity-scoped bonuses; their `contextNotes` (see `@pf1/engine`
 * `vigilante-talents.ts`) remind the player to apply them by hand while in the
 * matching identity, rather than promising enforcement this project doesn't
 * have a modeled "which identity is active right now" hook for anywhere else
 * in the sheet.
 */

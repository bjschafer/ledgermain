 ...
 * `setAntipaladinBoon` (B). Mirrors `doc.psychicDiscipline.test.ts`'s pattern
 * for `setPsychicDiscipline`, minus the trim/blank-string handling —
 * `antipaladinBoon` is a closed two-value union, not a free-text RefData tag,
 * so `null` is the only "clear" input.
 */

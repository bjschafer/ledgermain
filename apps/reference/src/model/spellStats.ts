import { formatDiceFormula, tryEvaluateFormula } from "@pf1/engine/formula";
 ...
 * `range`/`duration`/`area`/`damage`, plus `Spell.components`).
 *
 * Caller convention: range, area, and duration come from the FIRST action that
 * carries the field — a spell's `actions` are cast variants (heal-living vs.
 * harm-undead, ranged vs. melee), and the primary variant leads.
 */
...
export function spellDamageParts(spell: Spell, cl: number): SpellDamage[] {
  ...
  for (const part of parts) {
    ...
    if (text === null) {
      ...
      // Nothing resolved: drop the part rather than printing DSL source at a
      // reader who came here for the spell's rules text.
      if (n === null) continue;
      text = String(n);
    }
